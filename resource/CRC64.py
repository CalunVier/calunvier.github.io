class CRC64Hash:
    ...
    _seed = 0xFFFFFFFFFFFFFFFF
    digest_size = 8
    block_size = 1
    _crc_table = [0x0000000000000000, 0xb32e4cbe03a75f6f, 0xf4843657a840a05b, 0x47aa7ae9abe7ff34, 0x7bd0c384ff8f5e33, 0xc8fe8f3afc28015c, 0x8f54f5d357cffe68, 0x3c7ab96d5468a107,
    ...
    def __init__(self, data=None, seed=0xFFFFFFFFFFFFFFFF, poly=None, value=None):
        if poly is not None:
            self._poly = poly
            CRC64Hash.init_crc_table(self)
        self._seed = seed
        if value is None:
            self._value = seed
        else:
    ...
    # This is the algorithm without hash table
    # def update(self, data):
    #     for b in data:
    #         self._value = self._value ^ b
    #         for i in range(8):
    #             if self._value & 1:
    #                 self._value = (self._value >> 1) ^ self._poly
    #             else:
    #                 self._value = self._value >> 1

    def update(self, data):
        """Updates the CRC64 hash with the provided bytes"""
        if not isinstance(data, bytes):
            data = bytes(data)
        for byte in data:
            self._value = self._crc_table[(
                self._value ^ byte) & 0xFF] ^ (self._value >> 8)

    def digest(self):
        return (self._value ^ self._seed).to_bytes(self.digest_size, byteorder='big', signed=False)

    def hexdigest(self):
        return self.digest().hex()

    def copy(self):
        new_hash = CRC64Hash(seed=self._seed, poly=self._poly)
        return new_hash

#   This is the methord used to generate the hash table. If Poly changed, you need to call it first
    @classmethod
    def init_crc_table(cls):
        """Initializes the memoized lookup table for CRC64 calculation"""
        cls._crc_table = [0] * 256
        for i in range(256):
            crc = i
            for j in range(8):
                if crc & 1:
                    ...
                    crc >>= 1
            cls._crc_table[i] = crc

def crc64(data=None, seed=0xFFFFFFFFFFFFFFFF, poly=0xC96C5795D7870F42, value=None):
    return CRC64Hash(data, seed, poly, value)
